...
export async function autoAssignMonthly(
  ...
  for (const day of days) {
    const eligibleMain = active.filter((a) => canAssignShift(a.shift_type as ShiftType, day.slotType, day.weekday));
    ...
    const eligibleBackup = active.filter(
      (a) =>
        a.id !== mainId &&
        a.id !== poolingId &&
        canAssignShift(a.shift_type as ShiftType, day.slotType, day.weekday)
    );
